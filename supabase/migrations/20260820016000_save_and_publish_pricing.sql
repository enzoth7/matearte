-- Guarda y publica el catálogo completo en una sola transacción.
-- El versionado continúa existiendo para auditoría, pero la UI administrativa
-- expone una única acción operativa.
create or replace function public.save_and_publish_pricing(
  p_version_id uuid,
...
revoke all on function public.save_and_publish_pricing(uuid, timestamptz, jsonb) from public, anon, authenticated;
grant execute on function public.save_and_publish_pricing(uuid, timestamptz, jsonb) to authenticated;
