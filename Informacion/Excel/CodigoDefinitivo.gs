/**
 * SISTEMA CON PRIMARY KEY, INSPECCIÓN DE CLIENTE (COL B) Y DESPLAZAMIENTO EXCLUSIVO DE A:G (OPCIÓN 1)
 * --------------------------------------------------------------------------------------------------
...
function onEdit(e) {
  ...
  // 2. Procesar "Completado" en Producción (Columna F / 6 en Producción)
  if (sheetName === "Producción" && range.getColumn() === 6 && range.getRow() >= 10) {
    ...
    if (estado === "Completado") {
      ...
      // OPCIÓN 1: MOVER Y DESPLAZAR ÚNICAMENTE EL RANGO A:G (NO TOCA TABLAS DE LA DERECHA)
      const lastRowProd = sheet.getLastRow();
      
      if (lastRowProd > fila) {
        // Desplazar solo las columnas A a G de las filas inferiores un lugar hacia arriba
        ...
        // Si era el último pedido, vaciar únicamente las celdas A:G
