  ...
  return (
    <main id="contenido" className="section-space">
      <div className="container-shell max-w-3xl">
        <p className="eyebrow text-[var(--leather)]">Tu cuenta</p>
        <h1 className="display-md mt-7">Ingresá para ver tus pedidos</h1>
        <p className="mt-5 max-w-xl text-black/65">
          El acceso se realiza desde el visualizador y luego volvés a la tienda con una sesión segura.
        </p>
        <a className="button-primary mt-8" href={`${customizer}/?auth=login&next=profile`}>Continuar con Google</a>
      </div>
    </main>
  );
}
...
export default async function ProfilePage() {
  ...
  if (!user) return <AccountAccessRequired />;

  const [{ data: profile }, { data, error }] = await Promise.all([
    client
      .from("customer_profiles")
      .select("full_name,department,city,avatar_path")
      .eq("user_id", user.id)
      .maybeSingle(),
    client
      .from("orders")
      .select("id,order_number,status,total_minor,currency,created_at,paid_at,order_items(id,item_type,title,quantity,total_minor)")
      .eq("user_id", user.id)
        ...
        <section className="overflow-hidden border border-black/15 bg-[var(--paper)] shadow-[var(--shadow-soft)]" aria-labelledby="account-title">
          ...
          <div className="grid lg:grid-cols-[minmax(0,1fr)_21rem]">
            <div className="p-6 sm:p-8 lg:p-10">
              ...
              <a className="button-secondary mt-7 gap-2 sm:ml-[8.5rem]" href={`${customizer}/?view=profile`}>
                <PencilSimple size={18} aria-hidden="true" />
                Editar datos y diseños
              </a>
            </div>
                  ...
                  <article key={order.id} className="group overflow-hidden border border-black/15 bg-[var(--paper)] transition-[border-color,box-shadow] duration-200 hover:border-[var(--leather)]/45 hover:shadow-[var(--shadow-soft)]">
                    ...
                    <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[minmax(0,1fr)_15rem] lg:items-center">
                      ...
                      <div className="flex items-center justify-between gap-5 border-t border-black/10 pt-5 lg:block lg:border-l lg:border-t-0 lg:py-1 lg:pl-7 lg:text-right">
                        <div>
                          <p className="text-[0.68rem] font-semibold tracking-[0.12em] text-black/45 uppercase">Total</p>
