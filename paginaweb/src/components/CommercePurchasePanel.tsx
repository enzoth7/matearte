...
export function CommercePurchasePanel({ slug }: { slug: string }) {
  ...
  const [selected, setSelected] = useState(""); const [message, setMessage] = useState(""); const [added, setAdded] = useState(false); const [busy, setBusy] = useState(false);
  ...
  const add = async () => {
    ...
    if (response.status === 401) { addLocalCartItem(selected, 1, variant.option_values); setMessage(t("savedLocally")); setAdded(true); }
    else { await response.json(); if (response.ok) { setAdded(true); } else { setMessage(t("addFailed")); } }
    setBusy(false);
  };
  return <section className="mt-7 border border-black/20 bg-[var(--paper)] p-5"><label className="text-xs font-semibold uppercase tracking-widest">{t("variant")}<select className="mt-2 h-12 w-full border border-black/20 bg-transparent px-3 text-sm normal-case tracking-normal" value={selected} onChange={(e) => { setSelected(e.target.value); setAdded(false); setMessage(""); }}>{data.product.variants.map((item) => <option key={item.id} value={item.id}>{item.name} · $ {(item.price_minor / 100).toLocaleString("es-UY")} UYU</option>)}</select></label><p className="mt-4 display-font text-3xl">$ {(variant.price_minor / 100).toLocaleString("es-UY")} UYU</p>{!data.commerceEnabled && <p className="mt-3 text-xs text-amber-800">{t("catalogReady")}</p>}<button disabled={busy || !data.commerceEnabled} onClick={() => void add()} className="mt-5 min-h-12 w-full bg-[var(--walnut)] px-5 text-sm font-semibold text-white disabled:opacity-50">{busy ? t("adding") : added ? t("added") : t("addToCart")}</button>{added && <Link href="/carrito" className="mt-3 block text-center text-xs font-semibold underline underline-offset-2">{t("goToCart")}</Link>}{message && !added && <p role="status" className="mt-3 text-xs leading-5">{message}</p>}</section>;
}
