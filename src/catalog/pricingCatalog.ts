...
export const rawGoogleSheetPrices: ProductPrice[] = [
  ...
  { nro: 11, modelo: "Imperial", variante: "Cincelado a Lacre", virola: "Imperial", priceARS: 160000, priceUYU: 4340.75 },
  ...
  { nro: 17, modelo: "Torpedo", variante: "Alpaca Cincelado al Lacre", virola: "Alpaca Cincelado al Lacre", cuero: "Liso", priceARS: 90500, priceUYU: 2455.24 },
...
export function updateCustomizationPrice(id: string, priceUYU: number) {
  customizationPrices[id] = priceUYU;
}
...
export function updateVariantDetails(id: string, details: Partial<VariantDetails>) {
  variantDetailsMap[id] = {
    name: details.name || variantDetailsMap[id]?.name || "",
...
export function getVariantPrice(variantId: string) {
  const price = variantPriceMap[variantId] || { priceARS: 0, priceUYU: 0 };
  return {
    ...price,
