...
- Catálogo, zonas, revisión de personalizados y reembolsos: `commerce-admin`.
...
Las migraciones de `supabase/migrations` crean perfiles, diseños, assets privados, handoffs, catálogo, variantes, envíos, carritos, pedidos, pagos y membresías administrativas. Todas las tablas de clientes tienen grants explícitos y RLS por propietario. Los buckets `design-assets`, `design-previews` y `order-assets` son privados y usan rutas `user_id/design_id/...`.
...
3. Crear variantes con SKU, precio UYU, peso y modalidad.
...
- Pedido movido al estado correspondiente después de la confirmación del pago.
