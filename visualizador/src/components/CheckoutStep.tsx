...
export function CheckoutStep({ subtotalUYU, pricingReady, onBack, onContinue }: CheckoutStepProps) {
  ...
  const continueToStore = async () => {
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo abrir el carrito.'); setSubmitting(false); }
  };
  return (
    <main id="main-content" className="transaction-checkout">
      <section className="transaction-checkout__card">
        <h1>Continuá en el carrito</h1>
        <p className="transaction-checkout__price">
          {pricingReady ? `$ ${subtotalUYU.toLocaleString('es-UY')} UYU` : 'Precio pendiente'}
        </p>
        <p className="transaction-checkout__copy">En el carrito podés elegir la entrega, sumar otros productos y pagar.</p>
        ...
        <div className="transaction-checkout__actions">
          <button type="button" disabled={!pricingReady || submitting} onClick={() => void continueToStore()}>
            {submitting ? 'Abriendo carrito…' : 'Ir al carrito'}
          </button>
          <button type="button" onClick={onBack}>Volver al resumen</button>
        </div>
      </section>
    </main>
  );
