...
const start = code.indexOf('export async function fetchDashboardData');
const end = code.indexOf('export async function createCustomer');
const good = \export async function fetchDashboardData(): Promise<DashboardData> {
...
\;
fs.writeFileSync('src/lib/supabaseService.ts', code.substring(0, start) + good + code.substring(end));
